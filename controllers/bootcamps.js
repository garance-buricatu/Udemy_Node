...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
    // Add user to req.body (automatically fill user field in Bootcamp model)
    req.body.user = req.user.id; // NOTE: from auth/protect middleware, the req.user stores currently logged in user

    // Ensure publisher can only create 1 bootcamp
    const publishedBootcamp = await Bootcamp.findOne({ user: req.user });

    // If the user is not admin, they can only add 1 bootcamp
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
    ...
    // Make sure user who updates bootcamp also created the bootcamp (unless they are admin)
    if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin'){
        return next(new ErrorResponse(`User ${req.user.id} is not authorized to edit this bootcamp`, 404));
    }
